...
def gstreamer_pipeline(capture_width=1280, capture_height=720, display_width=1280, display_height=720, framerate=10,
                       flip_method=0):
    ...
    def draw_boxes_and_objects(self,boxes, img, cls_names, scores, num_detections):
        ...
        if self.objects:
            for key in self.objects.keys():
                ...
                img =cv2.line(img,(0, self.roi), (widthInput, self.roi),(0,0,255))
                # To use a vertical ROI
                #img = cv2.line(img, (self.roi, 0), (self.roi, heightInput), (0, 0, 255))
                # Reset 'Update' parameter of all the actual objects
                if self.objects[key]['Past'] ==False and self.objects[key]['Present'] ==True:
                    self.counter = self.counter+1
                    # vertical ROI
                    img = cv2.line(img, (0, self.roi), (widthInput, self.roi), (255, 0, 0))
                    #img = cv2.line(img, (self.roi, 0), (self.roi, heightInput), (255, 0, 0))
...
pb_fname = "./models/trt_cocovehicle_polen.pb"
print('Loading graph')
sta = time.time()
...
while cv2.getWindowProperty('Video', 0) >= 0:
    ...
    for i in range(num_detections):
        ...
        box = boxes[i] * np.array([heightInput, widthInput, heightInput, widthInput])
        box = np.round(box).astype(int)
